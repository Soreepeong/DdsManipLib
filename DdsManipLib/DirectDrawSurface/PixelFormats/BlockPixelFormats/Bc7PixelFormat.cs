...
public abstract class Bc7PixelFormat : BlockPixelFormat {
    ...
    private SquishOptions2 GetSquishOptions2(IRawPixelFormat fmt, SquishOptions2? template = default) {
        template ??= new();
        template.Method = SquishMethod.Bc7;
    ...
    protected Bc7PixelFormat() : base(AlphaType.None) { }
}

public sealed class Bc7TypelessPixelFormat : Bc7PixelFormat {
    public override DxgiFormat DxgiFormat => DxgiFormat.Bc7Typeless;

    public override IRawPixelFormat SuggestedRawPixelFormat => AlphaType == AlphaType.None
        ? new B8G8R8X8TypelessPixelFormat()
        : new R8G8B8A8TypelessPixelFormat(AlphaType);
}

public sealed class Bc7UNormPixelFormat : Bc7PixelFormat, IPixelFormat {
    public override DxgiFormat DxgiFormat => DxgiFormat.Bc7UNorm;

    public override IRawPixelFormat SuggestedRawPixelFormat => AlphaType == AlphaType.None
        ? new B8G8R8X8UNormPixelFormat()
        : new R8G8B8A8UNormPixelFormat(AlphaType);
}

public sealed class Bc7UNormSrgbPixelFormat : Bc7PixelFormat {
    public override DxgiFormat DxgiFormat => DxgiFormat.Bc7UNormSrgb;

    public override IRawPixelFormat SuggestedRawPixelFormat => AlphaType == AlphaType.None
        ? new B8G8R8X8UNormSrgbPixelFormat()
        : new R8G8B8A8UNormSrgbPixelFormat(AlphaType);
}
