...
public sealed class R8G8UNormPixelFormat : R8G8PixelFormat, IRawRgPixelFormat<byte> {
    ...
    public override void SetRed(Span<byte> pixel, float value) => pixel[OffsetR] = byte.CreateTruncating(Math.Clamp(value, 0, 1) * 256);
    public override void SetGreen(Span<byte> pixel, float value) => pixel[OffsetG] = byte.CreateTruncating(Math.Clamp(value, 0, 1) * 256);
