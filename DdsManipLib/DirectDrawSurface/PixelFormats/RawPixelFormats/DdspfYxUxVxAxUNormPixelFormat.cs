...
public sealed class DdspfYxUxVxAxUNormPixelFormat<T> : DdspfUNormPixelFormat, IRawYuvaPixelFormat<T>
    where T : unmanaged, IUnsignedNumber<T>, IBinaryInteger<T>, IMinMaxValue<T> {
    public DdspfYxUxVxAxUNormPixelFormat(int nbits, int yshift, int ybits, int ushift, int ubits, int vshift, int vbits, int ashift, int abits) : base(nbits,
        yshift,
        ybits,
        ushift,
        ubits,
    ...
    public override DdsPixelFormat DdsPixelFormat => DdsPixelFormat.FromYuva(BitsPerPixel, RedMax << RedShift, GreenMax << GreenShift, BlueMax << BlueShift, AlphaMax << AlphaShift);
