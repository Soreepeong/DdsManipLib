...
public sealed class R8G8SNormPixelFormat : R8G8PixelFormat, IRawRgPixelFormat<sbyte> {
    ...
    public override void SetRed(Span<byte> pixel, float value) => pixel[OffsetR] = (byte) sbyte.CreateSaturating(Math.Clamp(value, -1, 1) * 128);
    public override void SetGreen(Span<byte> pixel, float value) => pixel[OffsetG] = (byte) sbyte.CreateSaturating(Math.Clamp(value, -1, 1) * 128);
