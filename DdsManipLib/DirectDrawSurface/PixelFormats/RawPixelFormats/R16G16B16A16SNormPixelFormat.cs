...
public sealed class R16G16B16A16SNormPixelFormat : R16G16B16A16PixelFormat, IRawRgbPixelFormat<short>, IRawAPixelFormat<short> {
    ...
    public override void SetRed(Span<byte> pixel, float value) => SetRed(pixel, short.CreateSaturating(32768f * Math.Clamp(value, -1, 1)));
    public override void SetGreen(Span<byte> pixel, float value) => SetGreen(pixel, short.CreateSaturating(32768f * Math.Clamp(value, -1, 1)));
    public override void SetBlue(Span<byte> pixel, float value) => SetBlue(pixel, short.CreateSaturating(32768f * Math.Clamp(value, -1, 1)));
    public override void SetAlpha(Span<byte> pixel, float value) => SetAlpha(pixel, short.CreateSaturating(32768f * Math.Clamp(value, -1, 1)));
    ...
    public R16G16B16A16SNormPixelFormat(AlphaType alphaType) : base(alphaType) { }
}
