...
public sealed class R4G4UNormPixelFormat : RawRgPixelFormat, IRawRgPixelFormat<byte> {
    ...
    public override void SetRed(Span<byte> pixel, float value) => SetRed(pixel, byte.CreateSaturating(value * 256));
    public override void SetGreen(Span<byte> pixel, float value) => SetGreen(pixel, byte.CreateSaturating(value * 256));
    ...
    public void SetRg(Span<byte> pixel, Vector2 rg) => SetRg(pixel, new Vector2<byte>(byte.CreateSaturating(rg.X * 256), byte.CreateSaturating(rg.Y * 256)));
