...
public sealed class A4B4G4R4UNormPixelFormat : RawRgbaPixelFormat, IRawRgbaPixelFormat<byte> {
    ...
    public override float GetRed(ReadOnlySpan<byte> pixel) => (pixel[1] & 0xF0) / 240f;
    public override float GetGreen(ReadOnlySpan<byte> pixel) => (pixel[1] & 0x0F) / 15f;
    public override float GetBlue(ReadOnlySpan<byte> pixel) => (pixel[0] & 0xF0) / 240f;
    public override float GetAlpha(ReadOnlySpan<byte> pixel) => (pixel[0] & 0x0F) / 15f;
    public byte GetRedTyped(ReadOnlySpan<byte> pixel) => (byte) ((pixel[1] & 0xF0) >> 4);
    public byte GetGreenTyped(ReadOnlySpan<byte> pixel) => (byte) ((pixel[1] & 0x0F) >> 0);
    public byte GetBlueTyped(ReadOnlySpan<byte> pixel) => (byte) ((pixel[0] & 0xF0) >> 4);
    public byte GetAlphaTyped(ReadOnlySpan<byte> pixel) => (byte) ((pixel[0] & 0x0F) >> 0);
    ...
    public void SetRed(Span<byte> pixel, byte value) => pixel[1] = (byte) ((pixel[1] & ~0xF0) | (byte.Clamp(value, 0, 0xF) << 4));
    public void SetGreen(Span<byte> pixel, byte value) => pixel[0] = (byte) ((pixel[1] & ~0x0F) | (byte.Clamp(value, 0, 0xF) << 0));
    public void SetBlue(Span<byte> pixel, byte value) => pixel[0] = (byte) ((pixel[0] & ~0xF0) | (byte.Clamp(value, 0, 0xF) << 4));
    public void SetAlpha(Span<byte> pixel, byte value) => pixel[1] = (byte) ((pixel[0] & ~0x0F) | (byte.Clamp(value, 0, 0xF) << 0));

    public void SetRgba(Span<byte> pixel, Vector4<byte> rgba) {
        pixel[0] = (byte) (byte.Clamp(rgba.W, 0, 15) | (byte.Clamp(rgba.Z, 0, 15) << 4));
        pixel[1] = (byte) (byte.Clamp(rgba.Y, 0, 15) | (byte.Clamp(rgba.X, 0, 15) << 4));
    }
